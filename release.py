...
from shared.version import VERSION, VERSION_INFO  # noqa: E402
...
# ── 打包排除 ──
SKIP_DIRS = {
    "__pycache__", ".git", "logs", "backups", ".workbuddy",
    "archives", "dist", "tests",
    "Server",  # v3.05.00: BDS 服务器程序，不应打包
}
SKIP_FILES = {
    # 用户配置 + 运行时数据（不应覆盖）
    "bds_manager_config.json",
    "bds_version_cache.json",
    "bds_manager_notifications.json",  # v3.02.00 通知中心数据
    # 本地开发工具（不需要给最终用户）
    ".gitignore", "release.py", "archive.py",
}
...
def check_clean_workspace() -> bool:
    ...
    使用 -uno 忽略 untracked 文件（已 .gitignore 的 bds_manager_config.json 等）。
    """
    r = run(["git", "status", "--porcelain", "-uno"])
    if r.stdout.strip():
        err("工作区不干净，请先提交所有更改:")
        print(r.stdout)
        return False
...
# ── version.json ──
def write_version_json(sha256: str, size: int):
    """覆盖写 version.json 的发布字段，保留 min_compatible_version / changelog 不动。

    ⚠️ min_compatible_version 和 changelog 由开发者手动维护（用编辑器改 version.json），
    不应该被 release.py 覆盖：
      - min_compatible_version 是兼容性策略决策（如跨主版本升级窗口）
      - changelog 应该写得详细（标题 + bullet），命令行 --notes 太简陋
    """
    data = {}
...
def main() -> int:
    ...
    write_version_json(sha, size)
    ok("version.json 已写入（min_compatible_version / changelog 保留开发者手动设置）")
