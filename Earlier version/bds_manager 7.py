...
from PyQt5.QtGui import QFont, QColor, QTextCursor, QIcon, QPainter
...
class ToastNotification(QWidget):
    ...
    def __init__(self, parent, title, message, level="info", duration=4000):
        super().__init__(None)  # 无父窗口，独立定位
        self._window = parent  # 引用主窗口用于坐标计算
        ...
        colors = {
            "error": ("#f25f5c", "rgba(40,12,12,0.92)", "❌"),
            "warning": ("#f7a42c", "rgba(40,28,8,0.92)", "⚠️"),
            "success": ("#3ecf8e", "rgba(8,32,20,0.92)", "✅"),
            "info": ("#5c8aff", "rgba(8,18,40,0.92)", "ℹ️"),
        }
        ...
        title_label.setStyleSheet(f"font-weight:bold; font-size:12px; color:{accent}; background:transparent;")
        msg_label = QLabel(message)
        msg_label.setWordWrap(True)
        msg_label.setStyleSheet("font-size:11px; color:#aabbcc; background:transparent;")
    ...
    def _start_slide_in(self):
        # 从窗口右边缘外滑入
        right_edge = self._window.mapToGlobal(QPoint(self._window.width(), 0)).x()
    ...
    def _dismiss(self):
        ...
        self._anim_out.setEndValue(QPoint(end_x + 50, self.y()))
